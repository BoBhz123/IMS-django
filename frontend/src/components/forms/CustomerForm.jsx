...
function CustomerFormBody({ onClose, onSaved, customer }) {
  ...
  const [form, setForm] = useState(() =>
    customer
      ? { name: customer.name, phone_number: customer.phone_number ?? '', location: customer.location ?? '' }
      : emptyForm,
  )
  ...
  async function handleSubmit(event) {
    ...
    try {
      if (isEdit) {
        await api.patch(`/inventory/customers/${customer.id}/`, form)
      } else {
        created = (await api.post('/inventory/customers/', form)).data
      }
      onSaved(created)
      onClose()
    } catch (error) {
